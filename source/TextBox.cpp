...
void TextBox::update_text() {
    if (text_setting) {
        ...
        Vector2 text_size = MeasureTextEx(text_setting->font, m_text.substr(0, m_cursor_index).c_str(), text_setting->font_size, text_setting->spacing);
        if (!text_size.y) text_size.y = text_setting->font_size;
        m_text_position.x += 5;
