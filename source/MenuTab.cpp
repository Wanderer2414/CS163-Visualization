...
void ButtonTab::draw() {
    ...
    if (m_is_showed) {
        for (int i = 0; i < Items.size(); i++) {
            ...
            DrawRectangleRounded({ m_position.x, m_position.y + (m_size.y+2) * (i + 1), m_size.x, m_size.y }, button_setting->roundness, button_setting->segment, rectColor);
            DrawTextEx(text_setting->font, Items[i].c_str(),
                { m_position.x + m_size.x / 2 - Length[i] / 2,
                m_position.y + (m_size.y+2) * (i + 1) + m_size.y / 2 - 15}, text_setting->font_size, text_setting->spacing, text_setting->color);
        }
    }
}
ButtonTab::~ButtonTab()
